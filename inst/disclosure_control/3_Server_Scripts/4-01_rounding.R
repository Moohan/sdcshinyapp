...
shiny::observeEvent(input$rounding, {
  if (is.null(input$Disc_Variables_Round)) {
    shinyalert::shinyalert("No variables selected", "Please select input variables.", type = "error")
    shiny::validate(shiny::need(input$Disc_Variables_Round, "No variables selected for rounding"))
  }
