...
shiny::observe({
  ...
  shiny::updateSelectInput(session, "Disc_Variables_Pri_Supp", "Choose Variables for Primary Suppression:", cb_options, "")
})

shiny::observe({
  cb_options <- sdcshinyapp::SelectBox_Update(App_data$values)
  shiny::updateSelectInput(session, "Disc_Variables_Secondary_Supp", "Choose Variables for Secondary Suppression:", cb_options, "")
})
...
shiny::observeEvent(input$pri_sup, {
  if (is.null(input$Disc_Variables_Pri_Supp)) {
    ...
    shiny::validate(shiny::need(input$Disc_Variables_Pri_Supp, "No variables selected for primary suppression"))
  }

  # Validate Pri_Supp_Cond is a positive whole integer
  if (is.null(input$Pri_Supp_Cond) || !is.numeric(input$Pri_Supp_Cond) || input$Pri_Supp_Cond <= 0 || input$Pri_Supp_Cond %% 1 != 0) {
    shinyalert::shinyalert("Invalid suppression condition", "Primary suppression condition must be a positive whole number.", type = "error")
...
shiny::observeEvent(input$pri_sec_sup, {
  if (
    ...
    shiny::validate(
      shiny::need(input$Disc_Variables_Pri_Supp, "No primary suppression variable selected"),
      shiny::need(length(input$Disc_Variables_Pri_Supp) == 1, "Select exactly one primary suppression variable"),
      shiny::need(input$Disc_Variables_Secondary_Supp, "No secondary suppression variables selected"),
      shiny::need(length(input$Disc_Variables_Secondary_Supp) >= 2, "Select at least two secondary suppression variables")
    )
  }
